#!/usr/bin/env python3
"""Enhanced KDE via Algebraic Diversity (v31).

v31 change: the improved Sheather-Jones (Botev) bandwidth is computed by a direct
implementation of the Botev-Grotowski-Kroese (2010) fixed point, embedded below,
replacing the KDEpy library call used through v30. The KDEpy implementation was
found to undersmooth systematically (h about 2.6x below the fixed point's value on
smooth targets, e.g. 0.075 vs 0.198 on a standard normal at n=5000, where the AMISE
optimum is 0.193), which misrepresented the Botev baseline in the tables and
figures. Only the Botev column and the Botev figure curves change in v31; every
other column reproduces v30 exactly (shared RNG stream, unchanged code paths).
KDEpy is no longer a dependency.
...
(LMMSE) taper, after the published AD+AR method (framework paper and the
...
def botev_isj_bandwidth(data, n_grid=2**14):
    """Improved Sheather-Jones bandwidth by the Botev-Grotowski-Kroese (2010)
    fixed point (direct port of the reference kde.m). Gaussian kernel, in data
    units. Falls back to Silverman's value if the fixed point cannot be
    bracketed (degenerate or tiny samples)."""
    d = np.asarray(data, float)
    N = len(np.unique(d))
    lo, hi = d.min(), d.max(); R = hi - lo
    if not np.isfinite(R) or R <= 0 or N < 4: return _h_silverman_fallback(d)
...
def h_isj(d):                                   # Botev improved Sheather-Jones bandwidth (direct fixed point)
    try: return float(botev_isj_bandwidth(d))
