...
To use a richer backend, register it before running an experiment:
    ...
    P.register("mixture",       my_gemmule_wrapper)        # GEMMULE, used under its GPL license

The public GitHub image ships ONLY this file (defaults + registry) plus plugins_example.py. The
private bundle additionally provides reference/wire_reference.py, which registers the AD BMG
reference engine and a GEMMULE wrapper. Neither the BMG engine nor GEMMULE is part of the public
image: a practitioner must supply their own blind-group-matching backend and/or group library,
and must obtain and use GEMMULE under its GPL license.
"""
import numpy as np
...
# selection (no external dependency). Register GEMMULE for the full cross-family
# library (35 families, model-selected order) under its GPL license.
# ---------------------------------------------------------------------------
def _gauss(x, m, v):
...
def fit_mixture(d, kmax=6):
    ...
    EM with BIC-selected order. Register a 'mixture' backend (e.g. GEMMULE, GPL) for the full
    cross-family library that also covers skew-normal and heavy-tailed components."""
